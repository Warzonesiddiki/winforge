...
func isolate(t *testing.T) {
	t.Helper()
	// Use a short fixed-prefix dir instead of t.TempDir(): subtests named with a
	// 300-character feature name would otherwise exceed the 255-byte path
	// component limit on Linux/macOS filesystems.
	dir, err := os.MkdirTemp("", "winforge-cli-test")
	if err != nil {
		t.Fatalf("creating temp dir: %v", err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(dir) })
	t.Setenv("WINFORGE_DATA_DIR", dir)
